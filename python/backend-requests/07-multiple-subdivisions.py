...
def main() -> None:
    try:
        print("Validating multiple subdivision codes: US-CA, US-NY, US-TX (country: US)")
        result = call_api("/api/validate/subdivision", {
            "country": "US",
        ...
        if result.get("results") and any(not r.get("valid") for r in result["results"]):
            raise ValueError("Some subdivisions are invalid")
