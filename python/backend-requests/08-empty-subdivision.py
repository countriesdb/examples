...
def main() -> None:
    try:
        print("Validating empty subdivision code for country: AQ")
        result = call_api("/api/validate/subdivision", {"country": "AQ", "code": ""})
        print("Response:", json.dumps(result, indent=2))
        
        if not result.get("valid"):
            raise ValueError("Empty subdivision validation failed")
