## Backend Example – Python (`urllib.request`)

Uses Python 3.8+ with native `urllib.request` (no external dependencies) to call the CountriesDB validation endpoints.
...
# No pip install needed - urllib.request is part of Python standard library
```
...
- `urllib.request` is part of Python's standard library, so no external dependencies are required.
...
- HTTP errors are caught and displayed with status codes and response bodies.
