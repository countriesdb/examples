"""Multiple country validation example using countriesdb-validator package"""
...
def main():
    try:
        codes = ['US', 'CA', 'MX', 'FR']
        print(f'Validating country codes: {codes}')
        results = validator.validate_countries(codes)
        print('Response:', json.dumps(results, indent=2))
        
        for result in results:
            if not result['valid']:
                raise ValueError(f"Country {result.get('code')} validation failed: {result.get('message')}")
